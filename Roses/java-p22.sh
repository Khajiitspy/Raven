...
server_up() {
    echo "Server up..."
    docker pull magicabyss/p22-java:latest
    docker stop p22-java_container || true
    docker rm p22-java_container || true
    docker run -d --restart=always -v /volumes/p22-java/images:/app/images --name p22-java_container -p 6892:8081 magicabyss/p22-java
}
